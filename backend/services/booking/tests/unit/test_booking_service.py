...
from backend.services.booking.models.booking import Booking, BookingStatus, Trip
...
@pytest.mark.asyncio
async def test_assign_driver_success(booking_service: BookingService, mock_repo: AsyncMock, mock_redis: AsyncMock) -> None:
...
@pytest.mark.asyncio
async def test_assign_driver_lock_failed(booking_service: BookingService, mock_repo: AsyncMock, mock_redis: AsyncMock) -> None:
    booking_id = uuid.uuid4()
...
@pytest.mark.asyncio
async def test_start_trip_invalid_state(booking_service: BookingService, mock_repo: AsyncMock) -> None:
    booking_id = uuid.uuid4()
