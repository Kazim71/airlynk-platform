...
async def _process_event(user_id: UUID, event_type: str, context: dict, event_id: str, n_type: NotificationType):
...
@celery_app.task(name="notification.process_event", bind=True, max_retries=3)
def process_notification_event(self, user_id: str, event_type: str, context: dict, event_id: str, n_type: str):
...
@celery_app.task(name="notification.deliver", bind=True, max_retries=3, acks_late=True)
