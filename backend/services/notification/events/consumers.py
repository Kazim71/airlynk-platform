...
async def handle_booking_event(envelope: EventEnvelope) -> None:
    ...
    passenger_id = data.get("customer_id") or data.get("passenger_id") or data.get("user_id")
    if not passenger_id:
        return
    process_notification_event.delay(
        user_id=str(passenger_id),
        event_type=envelope.event_name.value
        if hasattr(envelope.event_name, "value")
        else str(envelope.event_name),
        context={
            "title": "Booking Update",
            "message": f"Your booking {data.get('booking_id')} has been updated.",
        },
...
def init_notification_consumers() -> None:
    ...
    async def setup() -> None:
        async def _handle_booking_payload(payload: dict[str, Any]) -> None:
            envelope = EventEnvelope(**payload)
            await handle_booking_event(envelope)

        await start_consumer(
            "notification_booking_queue",
            ["booking.trip.created", "booking.trip.assigned"],
            _handle_booking_payload,
        )

        async def _handle_dispatch_payload(payload: dict[str, Any]) -> None:
            envelope = EventEnvelope(**payload)
            await handle_dispatch_event(envelope)

        await start_consumer(
            "notification_dispatch_queue",
            ["dispatch.driver.selected", "dispatch.assignment.confirmed"],
            _handle_dispatch_payload,
        )
