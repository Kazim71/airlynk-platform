    ...
    async with httpx.AsyncClient(base_url=BASE_URL, timeout=60.0) as client:
            ...
            if r.status_code == 200:
                ...
                if r.status_code == 200:
                    ...
                    report(
                        "Login Customer",
                        True,
                        f"Used seeded customer, token={customer_token[:20]}...",
                    )
                else:
        ...
        try:
            ...
            if geo_r.status_code == 200:
                ...
                duration_mins = int(
                    geo_data.get("duration_minutes", geo_data.get("duration_mins", 40))
                )
                report(
                    "Geo Estimate", True, f"distance={distance_km}km, duration={duration_mins}min"
                )
            else:
                report(
                    "Geo Estimate", False, f"status={geo_r.status_code}, body={geo_r.text[:200]}"
                )
                distance_km = 20.0
                duration_mins = 40

            # Then pricing estimate
            pricing_r = await client.post(
                "/pricing/estimate",
                json={
                    "pickup_lat": pickup_lat,
                    "pickup_lng": pickup_lng,
            ...
            if pricing_r.status_code == 200:
                ...
                report(
                    "Pricing Estimate",
                    False,
                    f"status={pricing_r.status_code}, body={pricing_r.text[:200]}",
                )
                total_fare = 500.0
        except Exception as e:
        ...
        try:
            ...
            if r.status_code == 201:
                ...
                report(
                    "Create Booking",
                    True,
                    f"id={booking_id}, price=INR {booking_price}, status={booking_data.get('booking_status')}",
                )
            else:
        ...
        try:
            ...
            if r.status_code in [200, 201]:
                ...
                report(
                    "Create Payment Intent", False, f"status={r.status_code}, body={r.text[:200]}"
                )
        except Exception as e:
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                report(
                    "Fleet Drivers List",
                    isinstance(drivers_data, list) and len(drivers_data) > 0,
                ...
                if isinstance(drivers_data, list) and len(drivers_data) > 0:
                    ...
                    if vehicles:
                        ...
                        vr = await client.get(
                            f"/fleet/vehicles?driver_id={driver_id}", headers=operator_headers
                        )
                    ...
                    if vehicle_id:
                        ...
                        if r.status_code == 200:
                            ...
                            assigned_driver_email = "driver1@airlynk.in"  # First driver
                        else:
                            # Check if dispatch already assigned
                            booking_r = await client.get(
                                f"/bookings/{booking_id}", headers=operator_headers
                            )
                            if booking_r.status_code == 200:
                                ...
                                if current_status in ["driver_assigned", "DRIVER_ASSIGNED"]:
                                    report(
                                        "Assign Driver",
                                        True,
                                        f"Already assigned by dispatch, status={current_status}",
                                    )
                                    ...
                                    report(
                                        "Assign Driver",
                                        False,
                                        f"status={r.status_code}, booking_status={current_status}, body={r.text[:200]}",
                                    )
                            else:
                                report(
                                    "Assign Driver",
            ...
            try:
                ...
                for i in range(1, 21):
                    ...
                    if r.status_code == 200:
                        ...
                        start_r = await client.patch(
                            f"/bookings/{booking_id}/start", headers=tmp_headers
                        )
                        if start_r.status_code == 200:
                            ...
                            report(
                                "Start Trip",
                                True,
                                f"Started with driver{i}, status={start_r.status_code}",
                            )
                            break
                            ...
                            report(
                                "Start Trip",
                                False,
                                f"driver{i}: status={start_r.status_code}, body={start_r.text[:200]}",
                            )
                            break
